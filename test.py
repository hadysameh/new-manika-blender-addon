...
def reset_bone_rotation(arm_pose_bone):
        arm_pose_bone.rotation_euler[0] =0
        arm_pose_bone.rotation_euler[1] =0
        arm_pose_bone.rotation_euler[2] =0
...
def get_bone_local_axes(armature_name, bone_name):
    """
...
def animate_arm(armature_name,arm_bone_name,arm_bone_degree_angles):
    ...
    global_x, global_y, global_z = get_bone_global_axes(armature_name, arm_bone_name)
    # Convert the custom axis to the bone's local space
    custom_x_axis_local = global_x @ pose_bone.matrix.to_3x3().inverted()
    custom_z_axis_local = global_z @ pose_bone.matrix.to_3x3().inverted()
...
def animate_with_arduino_data(arduino_data):
        ...
        # selected_bone_in_pose_mode.rotation_mode = "QUATERNION"
...
# draw_custom_axis(local_x,'local_x')
# draw_custom_axis(local_y,'local_y')
# draw_custom_axis(local_z,'local_z')
